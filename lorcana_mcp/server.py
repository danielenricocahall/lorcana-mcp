...
def create_server() -> FastMCP:
    ...
    @mcp.tool(
        description=(
            ...
            "type breakdown (Character/Action/Item/Song/Location), `keyword_counts` "
            "(copies per keyword — Evasive, Ward, Bodyguard, Shift, Singer, ...), "
            "`subtype_counts` (copies per subtype — Floodborn, Hero, Puppy, ...), and "
            "`card_keywords` (per-card keyword tags, so archetype questions like "
            "'how many Evasive bodies does this run?' are answerable without a "
            "follow-up search_cards call). "
            "Each entry is `{name: str, count: int}` where `name` is the card's "
